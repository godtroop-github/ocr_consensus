...
NAME_STOPWORDS = NAME_NOISE_TOKENS | {
    "自查",
    "龙信",
    "注销",
    "已注销",
...
class BusinessCheckScene(Scene):
    ...
    def _extract_name(merged_lines: list, sep_idx: int, filename_name: str = "") -> str:
        """从分隔符附近提取中文姓名"""
        filename_name = BusinessCheckScene._clean_person_candidate(filename_name)
        ocr_name = BusinessCheckScene._extract_name_from_lines(merged_lines, sep_idx)

        if filename_name and ocr_name:
            return ocr_name if BusinessCheckScene._is_suspicious_person_name(filename_name) else filename_name
        if filename_name:
    ...
    def _extract_name_from_filename(filename: str) -> str:
        ...
        for seg in re.split(r"[-_+\s]+", stem):
            ...
            candidates.extend(BusinessCheckScene._person_candidates_from_text(seg))
            m2 = re.match(r"^\d+([\u4e00-\u9fff]{2,8})$", seg)
            if m2:
                candidates.extend(BusinessCheckScene._person_candidates_from_text(m2.group(1)))
            if seg.startswith("投资任职信息") and len(seg) > 5:
                tail = seg[5:]
                candidates.extend(BusinessCheckScene._person_candidates_from_text(tail))

        # 兜底：直接抽取中文片段
        if not candidates:
            candidates = BusinessCheckScene._person_candidates_from_text(stem)
    ...
    @staticmethod
    def _person_candidates_from_text(text: str) -> List[str]:
        ...
        chunks = re.findall(r"[\u4e00-\u9fff]{2,8}", cleaned)
        candidates = []
        for chunk in chunks:
            name = BusinessCheckScene._clean_person_candidate(chunk)
            if name:
    ...
    def _clean_person_candidate(value: str) -> str:
        ...
        for chunk in reversed(chunks):
            if BusinessCheckScene._is_valid_person_name(chunk):
                return chunk
        return ""

    @staticmethod
    def _is_suspicious_person_name(value: str) -> bool:
        name = str(value or "").strip()
        return not BusinessCheckScene._is_valid_person_name(name)
    ...
    def _is_valid_person_name(value: str) -> bool:
        ...
        if name in NAME_STOPWORDS:
            return False
        if any(token in name for token in NAME_STOPWORDS if len(token) >= 2):
            return False
        return True
    ...
    @staticmethod
    def _extract_name_from_lines(merged_lines: list, sep_idx: int) -> str:
        if not merged_lines:
            return ""

        windows: List[Tuple[int, int]] = []
        if sep_idx >= 0:
            windows.append((sep_idx + 1, min(len(merged_lines), sep_idx + 7)))
            windows.append((max(0, sep_idx - 3), sep_idx))
        windows.append((0, min(len(merged_lines), 12)))
        windows.append((0, len(merged_lines)))

        for start, end in windows:
            for line in merged_lines[start:end]:
                text = str(line or "").strip()
                if BusinessCheckScene._line_is_name_noise(text):
                    continue
                cleaned = re.sub(r"^[.。·•丨|｜\[\]【】「」『』()（）\s]+", "", text)
                cleaned = re.sub(r"[.。·•丨|｜\[\]【】「」『』()（）\s]+$", "", cleaned)
                name = BusinessCheckScene._clean_person_candidate(cleaned)
                if name:
                    return name
        return ""
