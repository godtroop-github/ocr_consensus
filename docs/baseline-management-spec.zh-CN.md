...
基线比对列表页用于发现问题和筛选问题。它负责选择基线、选择被比较任务、执行比对、筛选差异记录和进入单条详情。

基线差异详情页用于单条记录核验。它只展示当前选中记录的共识结构化数据、任职明细左右对比和图片证据，不重新承担全量比对入口职责。

### 16.2 比对会话状态

一次基线比对生成后，应形成前端比对会话。会话内容包括：
...
详情页优先读取该比对会话。只有会话不存在、会话不匹配或浏览器缓存不可用时，详情页才回退调用 `/api/compare` 重新生成结果。

### 16.3 返回和翻页规则

从详情页返回列表页时，应恢复上一轮比对结果、筛选条件和筛选后的记录集合，不应要求用户重新点击“开始比对”。

详情页上一条/下一条必须基于列表页当前筛选后的记录集合翻页，而不是全量比对记录。这样用户筛选“待复核”后，详情页只在待复核记录之间切换。

### 16.4 详情页信息架构

详情页主题是数据核验，图片只是证据。默认布局采用左右两栏：

- 左侧主栏：审阅摘要、基础信息左右对比、任职明细左右对比。
- 右侧证据栏：基线图片、本次图片、多图切换和点击放大。

图片默认尺寸应控制在一屏内，避免遮挡基础信息和任职明细。图片支持点击放大，但放大后不应超过页面宽度。

### 16.5 新增和减少记录展示

如果记录属于新增或减少，不再逐字段生成无意义差异。详情页展示单侧共识结果和单侧任职明细，并将该记录标记为信息类变化。

### 16.6 任职明细详情展示

任职明细详情先做企业级对齐，再展示字段级差异。展示顺序为：

1. 企业是否匹配、新增、减少或变化。
2. 匹配企业下的企业名称、经营状态、承担职务、持股比例、来源图片逐字段左右对比。
3. 未匹配企业只展示单侧数据，避免制造伪冲突。

### 16.7 详情页截图时间核验规则

差异详情页中，截图时间不是普通业务字段。对同一人员记录进行基线与本次比对时：

1. 如果截图日期和截图时间完全一致，标记为疑点数据，提示可能是重复截图、重复上传或未更新采集证据。
2. 如果截图日期或截图时间发生变化，默认视为合理采集变化，使用绿色状态展示。
3. 截图时间不应和姓名、身份证号、企业数量、任职明细等强业务字段使用同一套变化风险判断。

### 16.8 详情页并排比较规则

详情页应同时满足两类并排比较：

1. 图片证据并排：基线批次图片和本次批次图片横向展示，支持各自多图切换。
2. 数据字段并排：基础信息和任职明细字段以“字段 / 基线 / 本次 / 核验”的列结构展示。

复核状态属于人工流程状态，不作为基线与本次数据的业务字段参与对比。

### 16.9 来源图片与证据提示规则

来源图片属于证据元数据，不属于业务字段，不参与基线与本次的字段差异判断。

在任职明细详情中，来源信息应以提示形式展示，内容包括：

- 数据来自哪个任务。
- 来源图片文件名。
- 该任务中对应企业的共识数据摘要：企业名称、经营状态、承担职务、持股比例。

来源提示用于追溯和定位证据，不应生成“来源图片不一致”的业务差异。

### 16.10 图片对齐规则

差异详情页中的图片用于肉眼对照同一页面位置的信息。默认应顶端对齐并完整缩放显示，避免不同批次截图在顶部区域出现错位。底部对齐不适合作为默认策略。

## 17. 真实基线更新 MVP

### 17.1 设计原则

真实基线不是某次任务结果的临时快照，而是可持续演进的数据资产。任务结果只能作为证据输入，不能无审计地覆盖真实基线。

第一阶段采用版本化更新策略：每次人工裁决都会生成一个新的基线版本，旧版本保留。若新版本类型为 `golden`，则新版本成为 active golden。
...
| 动作 | 含义 |
| --- | --- |
| 保留基线 | 新版本继续使用旧基线该记录数据 |
| 采用本次 | 新版本使用当前任务该记录共识数据 |
| 手工修正 | 新版本使用人工输入的基础信息字段，任职明细沿用当前记录 |

新增记录默认不会进入真实基线，除非人工选择采用本次或手工修正。

减少记录默认继续保留旧基线。若人工选择采用本次，则表示确认从新真实基线中移除该记录。

### 17.3 未裁决记录默认策略

生成新真实基线时，未裁决记录默认保留旧基线值。这样避免当前任务中的 OCR 偶发错误污染真实基线。

### 17.4 测试闭环

可验收路径：

1. 在基线管理页选择一个基线和一个已完成任务。
2. 开始比对。
3. 进入一条差异详情。
4. 选择保留基线、采用本次或手工修正。
5. 系统生成新的 active golden 基线版本。
6. 返回基线管理页，选择新基线继续比对。

### 17.5 后续增强

后续版本应扩展为字段级裁决和企业明细字段级裁决，并引入字段级证据计数、支持任务列表和可信度评分。

### 17.6 批量提交修订

真实基线更新应以一次比对会话为单位提交，而不是在详情页单条记录直接生成版本。

修订后的流程：

1. 列表页执行基线与任务比对。
2. 系统自动区分无需处理记录和待裁决记录。
3. 业务一致、仅截图时间变化、仅来源证据变化的记录无需人工裁决，也不应触发业务基线版本更新。
4. 业务字段不一致、新增、减少的记录进入待裁决。
5. 详情页只记录单条裁决，不直接落库。
6. 所有待裁决记录处理完成后，列表页统一提交并生成新的真实基线版本。

如果本次比对没有业务差异，则无需生成新的业务基线版本。后续可通过证据表单独积累 support_count、last_seen_task_id、last_seen_at 等观测信息。

### 17.7 待复核与待裁决口径

`待复核` 和 `待裁决` 是两套不同口径，不能要求数量一致。

| 口径 | 目的 | 统计对象 |
| --- | --- | --- |
| 待复核 | 识别质量稽核 | 存在 OCR/共识风险的记录 |
| 待裁决 | 真实基线更新 | 会影响真实基线业务值的字段或企业项 |

待裁决采用字段级统计：基础信息按字段统计，任职明细按企业字段统计，企业新增或减少按企业项统计。

### 17.8 字段级裁决

匹配记录不应只能整条采用基线或本次。详情页应允许字段级裁决：

- 基础信息字段：姓名、身份证号、相关企业、任职、参股。
- 任职明细字段：企业名称、经营状态、承担职务、持股比例。
- 企业新增或减少：按企业项裁决是否纳入或保留。

提交新真实基线时，系统按字段裁决结果拼装最终记录：
...
截图时间、来源图片、来源任务属于证据字段，不参与真实基线业务字段裁决。

### 17.9 行内裁决交互

字段级裁决控件应出现在差异发生的位置，而不是单独放在页面底部形成第二张裁决表。

详情页交互规则：

1. 基础信息不一致字段在该字段行内直接选择裁决。
2. 任职明细不一致字段在对应企业的字段行内直接选择裁决。
3. 企业新增或减少按企业项展示一个裁决控件，不再拆成多个无意义字段。
4. 页面底部只保留快捷操作和保存操作，例如本条全部保留基线、本条全部采用本次、保存字段裁决。
5. 新增记录的快捷操作文案应表达为接受全部进入基线或忽略新增。
6. 减少记录的快捷操作文案应表达为保留基线或确认移除。

列表页应提供待裁决和已裁决筛选，其中待裁决表示仍存在未完成业务字段/企业项裁决的记录，已裁决表示该记录所有待裁决字段/企业项均已处理。
