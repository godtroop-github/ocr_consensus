...
| 当前截图时间早于基线 | capture_time_older | 是，疑似旧截图 |
| 基线有时间，当前缺失 | capture_time_missing_current | 低优先级 |
| 基线缺失，当前有时间 | capture_time_filled | 否 |
| 时间格式非法 | capture_time_invalid | 是，OCR 质量问题 |
...
| 本次有、基线无 | image_added | 列表提示“图片增加” |
| 基线有、本次无 | image_missing | 列表提示“图片减少” |
| 图片集合变化但结构化字段不变 | image_changed_only | 进入图片变化筛选，建议复核 |
...
1. 基线批次和本次批次各自独立翻页。
2. 不做跨批次图片内容相似度判断，第一版只按文件名集合判断增加或减少。
3. 图片增加或减少不计入基础业务字段变化，但计入稽核提示。
4. 如果图片缺失导致结构化字段缺失，应由字段级 diff 共同触发复核。
...
4. 页面展示总记录、业务一致、业务变化、任职变化、图片变化、新增/缺失、仅时间变化、需复核等聚合指标。
5. 页面展示变化构成条和风险分布。
6. 页面支持按全部、需复核、业务变化、任职变化、图片变化、新增、缺失、疑似旧截图、仅时间变化快速切片。
7. 页面支持搜索、业务状态、采集时间状态、风险等级、只看需复核等精细筛选。
8. 页面左侧展示重点差异记录，包含编号、姓名、身份证号、截图时间、基础信息差异、任职明细差异、图片变化和操作入口。
9. 选中记录后，右侧展示基线批次图片、本次批次图片、基础信息共识结果和任职明细共识结果。
10. 基础字段差异采用“基线值 -> 本次值”的左右对照展示，便于人工稽核。
11. 任职明细差异按企业维度逐字段左右对照展示。
12. 基线详情区不再占用主页面空间，基线版本列表只负责选择和管理。
...
5. 当前图片变化只按文件名集合判断，不做图片内容 hash 或视觉相似度判断。
