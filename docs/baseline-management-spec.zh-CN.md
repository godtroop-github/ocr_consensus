...
1. 归一化 key 完全一致，直接匹配。
2. 一方是另一方的前缀或包含关系，且较短一方长度不少于 6 个字符，可视为同一企业。
3. 其他情况可使用字符串相似度阈值做保守匹配，避免 OCR 截断一个字导致“企业缺失 + 企业新增”的假差异。
4. 占位企业不参与宽松匹配。
...
1. 基线管理页不再展示独立“基线版本”框，基线选择统一由比对区下拉完成。
2. 基线管理页承担差异列表入口，不在本页承载记录详情。
3. 不展示“首条记录预览”，避免把样例预览误认为比对详情。
4. 不展示原始 OCR、模型置信度和方法级证据；这些属于任务质量分析，不属于基线差异稽核主流程。
...
5. 一侧有图、一侧无图，仅影响图片预览，不作为差异和复核条件。
6. 任职明细先展示公司级新增、缺失、变化、一致；展开公司后再展示企业名称、经营状态、承担职务、持股比例等字段的左右对比。
7. 新增记录和缺失记录只展示单侧共识结果，不再逐字段比较，也不进入待复核。
8. 差异列表和详情采用上下式主从布局，详情占满页面宽度，优先保证图片预览完整和逐项确认空间。
...
9. 页面用表格展示重点记录，包含编号、姓名、身份证号、截图时间、基础信息差异和任职明细差异。
10. 点击记录进入独立差异详情页，详情页展示基线批次图片、本次批次图片、基础信息共识结果和任职明细共识结果。
